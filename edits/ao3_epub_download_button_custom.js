...
// @version      3.0
// @description  Adds a customizable EPUB download button next to dates on works
...
(function () {
    ...
    function repositionButtons() {
        document.querySelectorAll(".ao3-epub-download-btn").forEach((btn) => btn.remove());
        addEpubButtons();
    }
        ...
        debounceTimer = setTimeout(() => {
            ...
            for (const mutation of mutations) {
                for (const node of mutation.addedNodes) {
                    ...
                    if (node.matches?.(".blurb") || node.querySelector?.(".header h4.heading")) {
