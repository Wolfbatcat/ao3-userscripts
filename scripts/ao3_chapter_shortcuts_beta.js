...
(function () {
  ...
  function showChapterShortcutsMenu() {
    ...
    const hideMenuCheckbox = helpers.createCheckbox({
      ...
      tooltip:
        "When checked, the 'Chapter Shortcuts' menu item will only appear on the main AO3 page to reduce menu clutter, but the script will still work on all pages.",
    });
    ...
    const addBottomCheckbox = helpers.createConditionalCheckbox({
      ...
      tooltip:
        "Show additional navigation and action buttons at the bottom of the page.",
    ...
    const buttons = helpers.createButtonGroup([
      {
        ...
        onClick: () => {
          ...
          CHAPTER_SHORTCUTS_CONFIG.addBottomButtons =
            helpers.getValue("add-bottom-buttons");
          CHAPTER_SHORTCUTS_CONFIG.bottomChapterButtons = helpers.getValue(
            "bottom-chapter-buttons",
          );
          CHAPTER_SHORTCUTS_CONFIG.bottomEntireWork =
            helpers.getValue("bottom-entire-work");
          CHAPTER_SHORTCUTS_CONFIG.bottomChapterIndex = helpers.getValue(
            "bottom-chapter-index",
          );
          CHAPTER_SHORTCUTS_CONFIG.bottomShare =
            helpers.getValue("bottom-share");
          CHAPTER_SHORTCUTS_CONFIG.bottomDownload =
            helpers.getValue("bottom-download");
    ...
    if (CHAPTER_SHORTCUTS_CONFIG.addBottomButtons) {
      ...
      if (actionsUl) {
        ...
        if (
          topLi &&
          topLi.parentElement &&
          CHAPTER_SHORTCUTS_CONFIG.bottomChapterButtons
        ) {
          ...
          const prevLi = Array.from(actionsUl.querySelectorAll("li")).find(
            (li) => {
          ...
          if (prevLi) {
            ...
            const bookmarkLi = actionsUl
              .querySelector("li .bookmark_form_placement_open")
              ?.closest("li");
            if (bookmarkLi) {
