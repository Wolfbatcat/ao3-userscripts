...
(function () {
    ...
    const VERSION = "4.2.3"; // Keep in sync with @version in userscript header
  ...
  `;
    ...
    function sanitizeConfig(config) {
        ...
        sanitized.conditionalTagBlacklist = Array.isArray(config.conditionalTagBlacklist)
    ...
    function createEyeToggle(filterId, initialState) {
        ...
        button.innerHTML = html`<span
            style="display:inline-block;width:1.2em;height:1.2em;vertical-align:-0.15em;background-color:currentColor;mask:url('${initialState
                ...
                ? ICON_HIDE
                : ICON_EYE}') no-repeat center/contain;"></span>`;
    ...
    function matchesCondition(condition, tags, exactMatch) {
        ...
        return condition.joinMode === "and" ? hits.every(Boolean) : hits.some(Boolean);
    }
        ...
        function getCategory(el) {
            if (el.tagName === "SPAN") {
                if (el.classList.contains("rating") || el.classList.contains("ratings"))
                    return "ratings";
                if (el.classList.contains("category") || el.classList.contains("categories"))
                    return "categories";
            }
            ...
            if (li) {
                if (li.classList.contains("rating") || li.classList.contains("ratings"))
                    return "ratings";
                if (li.classList.contains("warning") || li.classList.contains("warnings"))
                    return "warnings";
                if (li.classList.contains("category") || li.classList.contains("categories"))
                    return "categories";
                if (li.classList.contains("fandom") || li.classList.contains("fandoms"))
                    return "fandoms";
                if (li.classList.contains("relationship") || li.classList.contains("relationships"))
                    return "relationships";
                if (li.classList.contains("character") || li.classList.contains("characters"))
                    return "characters";
                if (li.classList.contains("freeform") || li.classList.contains("freeforms"))
                    return "freeforms";
            }
    ...
    function showQuickAddNotification(message) {
        const existing = document.getElementById("ao3-blocker-quickadd-notification");
    ...
    function clearProcessedFlags() {
        const processedWorks = document.querySelectorAll(`.${CSS_NAMESPACE}-processed`);
    ...
    function checkWorks() {
        ...
        blurbs.forEach((blurbEl) => {
            ...
            const blurbType = isSeries ? "series" : isCollection ? "collection" : "work";
            ...
            let reason = null;
            if (!isOnMyContent) reason = getBlockReason(blockables, config, blurbEl, blurbType);
            if (reason) {
    ...
    function handleQuickAdd(event) {
        ...
        if (target.classList.contains("tag")) {
            ...
            const useStrictBlacklist = config.enableStrictTagBlocking && event.shiftKey;
            const targetList = useStrictBlacklist ? "strictTagBlacklist" : "tagBlacklist";
            ...
            const alreadyExists = currentTags.some((t) => normalizeText(t) === normalizedTag);
            if (alreadyExists) {
                const listName = useStrictBlacklist ? "strict blacklist" : "tag blacklist";
            ...
            saveConfig(config);
            const rawField = useStrictBlacklist ? "_rawStrictTagBlacklist" : "_rawTagBlacklist";
            window.ao3Blocker.config[rawField] = updatedTags;
            window.ao3Blocker.config[targetList] = splitEntries(updatedTags).map((i) => {
            ...
            const listName = useStrictBlacklist ? "strict tag blacklist" : "tag blacklist";
        ...
        if (target.getAttribute("rel") === "author") {
            ...
            const updatedAuthors =
                currentAuthors.length > 0 ? config.authorBlacklist + ", " + authorText : authorText;
            ...
            for (let i = 0; i < config.tagHighlights.length; ++i) {
                ...
                if (item.type === "conditional") {
                    if (matchPattern(tagText, item.block, true)) {
                        const hasCondition = matchesCondition(item.condition, allTagTexts, true);
                        if (
    ...
    function initConfig() {
        if (WORKS_PAGE_REGEX.test(window.location.href)) {
            ...
            if (
                ...
                window.ao3Blocker.config.tagHighlights = splitEntries(config.tagHighlights).map(
                    (i) => {
                        const parsed = parseConditional(i);
                        if (parsed.operator) {
                            return {
        ...
        const rawConditionalTagBlacklist = JSON.stringify(config.conditionalTagBlacklist || []);
        ...
        const cacheValid =
            ...
            window.ao3Blocker.config._rawPrimaryRelationships === rawPrimaryRelationships &&
            ...
            window.ao3Blocker.config._rawConditionalTagBlacklist === rawConditionalTagBlacklist;
        ...
        if (!cacheValid) {
            window.ao3Blocker.config = {
                ...
                conditionalTagBlacklist: (config.conditionalTagBlacklist || []).map((rule) => ({
    ...
    function initSharedMenu() {
        ...
        if (menu) {
            ...
            if (
                ...
                settingsItem.querySelector("a").addEventListener("click", showBlockerMenu);
    ...
    function getFullTagBlacklistValue(config) {
        ...
        if (config.conditionalTagBlacklist && config.conditionalTagBlacklist.length > 0) {
    ...
    function showBlockerMenu() {
            ...
            window.AO3MenuHelpers.createMenuModal({
                content: html` <h3>Advanced Blocker: Filter Syntax</h3>
                    <p>
                        ...
                        summaries match anywhere automatically. Authors must be the full pseud
                        (capitalization ignored). Not every operator works in every field, as noted
                        below.
                    </p>
                    ...
                    <dl>
                        ...
                        <dd>
                            ...
                            <samp>Fix-It of Sorts</samp>, and <samp>Time Travel Fix-It</samp>. Works
                            in every field except <strong>Author</strong>, <strong>Work ID</strong>,
                            and <strong>Language</strong>, which need exact values.
                        </dd>
                    </dl>

                    <h4>Conditional Blocking</h4>
                    <p>
                        Block a tag depending on whether other tags are present, with the condition
                        inside <kbd>{ }</kbd>. Works in the tag fields (blacklist, strict blacklist,
                        whitelist, and highlight) and the primary relationship/character fields.
                        <strong>Not</strong> available in the title, summary, or author fields.
                    </p>
                    <dl>
                        ...
                        <dd>
                            <kbd>F/M unless:{Multi}</kbd> blocks <samp>F/M</samp> works that aren't
                            also tagged <samp>Multi</samp>.
                        </dd>

                        <dt>,: AND inside the braces (every condition must match)</dt>
                        <dd>
                            <kbd>Explicit with:{Dead Dove: Do Not Eat, Dubious Consent}</kbd> blocks
                            <samp>Explicit</samp> works only when they also have both
                            <samp>Dead Dove: Do Not Eat</samp> and <samp>Dubious Consent</samp>.
                        </dd>

                        <dt>||: OR inside the braces (any condition may match)</dt>
                        <dd>
                            <kbd>F/M unless:{Multi||M/M||F/F}</kbd> blocks <samp>F/M</samp> works
                            that aren't also tagged <samp>Multi</samp>, <samp>M/M</samp>, or
                            <samp>F/F</samp>.
                        </dd>
                    </dl>
                    ...
                    <p>
                        These fields keep a work only when one of your pairings or characters
                        appears within the Tag Window&mdash;the first few tags (default: 1 for
                        relationships, 5 for characters). Enter names exactly as they appear on AO3.
                        Here the conditional can only check fandoms: use <kbd>with:{Fandom}</kbd> to
                        limit a rule to one fandom, or <kbd>with:{Fandom1||Fandom2}</kbd> to cover
                        several.
                    </p>
                    <dl>
                        ...
                        <dt><kbd>*Harry Potter*</kbd></dt>
                        <dd>
                            allows <strong>all</strong> tags with Harry Potter, including platonic
                            ones like <samp>Harry Potter &amp; Sirius Black</samp>.
                        </dd>

                        <dt>
                            <kbd>Harry Potter/Tom Riddle with:{Harry Potter - J. K. Rowling}</kbd>
                        </dt>
                        <dd>
        ...
        const dialog = window.AO3MenuHelpers.createDialog("🛡️ Advanced Blocker 🛡️", {
            maxWidth: "800px",
            actions: [
                {
        ...
        function renderTagBlacklistUI(strictEnabled) {
            ...
            if (strictEnabled) {
                ...
                const strictInput = strictBlacklist.querySelector("#strict-tag-blacklist-input");
                if (strictInput) {
        ...
        const tagWhitelist = window.AO3MenuHelpers.createTextarea({
            ...
            placeholder: "*Happy Ending*, Fluff, Slow Burn* unless:{Enemies to Lovers}",
            tooltip:
        ...
        const tagHighlightsInput = window.AO3MenuHelpers.createTextarea({
            ...
            placeholder: "*Fix-It*, Enemies to Lovers, Angst with:{Happy Ending, Fluff}",
            tooltip: "Make these works stand out. * for wildcards. Conditionals supported.",
        });
        ...
        const boldTagsGroup = window.AO3MenuHelpers.createSettingGroup(boldTagsCheckbox);
        ...
        const pairingSection = window.AO3MenuHelpers.createSection("💕 Primary Pairing Filtering");
        ...
        const pairingRow = window.AO3MenuHelpers.createTwoColumnLayout(relPad, charPad);
        pairingSection.appendChild(pairingRow);
        dialog.appendChild(pairingSection);

        const workSection = window.AO3MenuHelpers.createSection("🔍 Work Filtering");
        ...
        const row1 = window.AO3MenuHelpers.createTwoColumnLayout(maxFandoms, maxMonths);
        ...
        const row2 = window.AO3MenuHelpers.createTwoColumnLayout(minWords, maxWords);
        ...
        const row3 = window.AO3MenuHelpers.createTwoColumnLayout(minChapters, maxChapters);
        ...
        const authorSection = window.AO3MenuHelpers.createSection("✏️ Author & Content Filtering");
        ...
        const showPlaceholdersCheckbox = window.AO3MenuHelpers.createConditionalCheckbox({
        ...
        const disableCollapseTransitionsCheckbox = window.AO3MenuHelpers.createCheckbox({
        ...
        const strictTagBlockingGroup =
            window.AO3MenuHelpers.createSettingGroup(strictTagBlockingCheckbox);
        ...
        if (config.enableStrictTagBlocking) {
            ...
            tipContent.appendChild(
                document.createTextNode(" to add to strict blacklist (hides completely)."),
            );
        }
        ...
        if (saveBtn)
            saveBtn.addEventListener("click", () => {
                ...
                const rawInput = window.AO3MenuHelpers.getValue("tag-blacklist-input") || "";
                const rawList = splitEntries(rawInput);
                const regular = [];
                const conditionals = [];
                rawList.forEach((entry) => {
                    // Check for conditional syntax: "Tag unless:{OtherTag}" or "Tag with:{OtherTag}"
                    const conditionalMatch = entry.match(/^(.+?)\s*(unless|with)\s*:\s*\{(.+)\}$/i);
                ...
                const updatedConfig = {
                    ...
                    tagWhitelist: window.AO3MenuHelpers.getValue("tag-whitelist-input") || "",
                    tagHighlights: window.AO3MenuHelpers.getValue("tag-highlights-input") || "",
                    authorBlacklist: window.AO3MenuHelpers.getValue("author-blacklist-input") || "",
                    titleBlacklist: window.AO3MenuHelpers.getValue("title-blacklist-input") || "",
                    summaryBlacklist:
                        window.AO3MenuHelpers.getValue("summary-blacklist-input") || "",
                    workBlacklist: window.AO3MenuHelpers.getValue("work-blacklist-input") || "",
                    showReasons: window.AO3MenuHelpers.getValue("show-reasons-checkbox"),
                    showPlaceholders: window.AO3MenuHelpers.getValue("show-placeholders-checkbox"),
                    highlightColor:
                    ...
                    maxCrossovers: window.AO3MenuHelpers.getValue("max-crossovers-input") || "",
                    ...
                    blockComplete: window.AO3MenuHelpers.getValue("block-complete-checkbox"),
                    ...
                    disableCollapseTransitions: (() => {
                        const el = document.getElementById("disable-collapse-transitions-checkbox");
        ...
        function updateEyeToggleVisibility() {
            const showPlaceholders = window.AO3MenuHelpers.getValue("show-placeholders-checkbox");
        ...
        const showPlaceholdersElement = dialog.querySelector("#show-placeholders-checkbox");
        if (showPlaceholdersElement) {
            showPlaceholdersElement.addEventListener("change", updateEyeToggleVisibility);
        }
        ...
        if (strictTagBlockingElement) {
            strictTagBlockingElement.addEventListener("change", function () {
                ...
                const strictElement = document.getElementById("strict-tag-blacklist-input");
                if (strictElement) {
    ...
    function getFold(reasons) {
        ...
        if (showReasons && reasons && reasons.length > 0) {
            ...
            reasons.forEach((reason) => {
                if (reason.completionStatus) parts.push(`<em>${reason.completionStatus}</em>`);
                ...
                if (reason.tags && reason.tags.length > 0) {
                    const categoryTags = new Set(["M/M", "Gen", "Multi", "F/F", "F/M", "Other"]);
                    ...
                    const otherTags = reason.tags.filter(
                        (tag) =>
                            !categoryTags.has(tag) && !ratingTags.has(tag) && !warningTags.has(tag),
                    );
                ...
                if (reason.language) parts.push(`<em>Language: ${reason.language}</em>`);
                if (reason.crossovers !== undefined)
                    parts.push(`<em>Fandoms: ${reason.crossovers}</em>`);
                if (reason.primaryPairing) parts.push(`<em>${reason.primaryPairing}</em>`);
            });
            message = parts.join("; ");
            iconHtml = html`<span
                class="${CSS_NAMESPACE}-icon"
                style="display:inline-block;width:1.2em;height:1.2em;vertical-align:-0.15em;margin-right:0.3em;background-color:currentColor;mask:url('${ICON_HIDE}') no-repeat center/contain;-webkit-mask:url('${ICON_HIDE}') no-repeat center/contain;"></span>`;
    ...
    function getToggleButton() {
        const showIcon = html`<span
            style="display:inline-block;width:1.2em;height:1.2em;vertical-align:-0.15em;margin-right:0.2em;background-color:currentColor;mask:url('${ICON_EYE}') no-repeat center/contain;-webkit-mask:url('${ICON_EYE}') no-repeat center/contain;"></span>`;
        const hideIcon = html`<span
            style="display:inline-block;width:1.2em;height:1.2em;vertical-align:-0.15em;margin-right:0.2em;background-color:currentColor;mask:url('${ICON_HIDE}') no-repeat center/contain;-webkit-mask:url('${ICON_HIDE}') no-repeat center/contain;"></span>`;
        const button = document.createElement("button");
        ...
        button.addEventListener("click", (event) => {
            ...
            const iconRegex = new RegExp(
                "<span[^>]*class=[\"']" + CSS_NAMESPACE + "-icon[\"'][^>]*><\\/span>\\s*",
                "i",
            );
            ...
            if (work.classList.contains(unhideClassFragment)) {
                ...
                note.innerHTML = html`<span
                        class="${CSS_NAMESPACE}-icon"
                        style="display:inline-block;width:1.2em;height:1.2em;vertical-align:-0.15em;margin-right:0.3em;background-color:currentColor;mask:url('${ICON_EYE}') no-repeat center/contain;-webkit-mask:url('${ICON_EYE}') no-repeat center/contain;"></span
                    >${message}`;
    ...
    function blockWork(workElement, reasons, config) {
        ...
        const hideCompletely = reasons.some(
            (reason) =>
                config.hideCompletelyRules && config.hideCompletelyRules[reason._filterType],
        );
        ...
        if (hideCompletely) {
            ...
            if (!config.showReasons) workElement.classList.add(`${CSS_NAMESPACE}-hide-reasons`);
        }
    }
    ...
    function checkPrimaryPairing(categorizedTags, config) {
        ...
        if (primaryRelationships.length === 0 && primaryCharacters.length === 0) return null;
        ...
        function getConditionInfo(entry) {
            ...
            if (entry.condition.conditions) {
                return {
                    key:
                        JSON.stringify(entry.condition.conditions.map((c) => c.text || "")) +
                        "|" +
        ...
        function conditionMatchesFandom(condObj, fandomTags) {
            ...
            if (condObj.conditions) return matchesCondition(condObj, fandomTags, true);
        ...
        if (charGroups.size > 0 && !characterGroupSatisfied) {
            return { primaryPairing: `Missing character(s)`, _filterType: "primaryCharacters" };
        }
    ...
    function getBlockReason(blockables, config, _blurbElement, blurbType) {
                ...
                config.strictTagBlacklist.forEach((item) => {
                    if (item.type === "simple") {
                        ...
                        if (matchPattern(tag, item.block, true)) {
                            ...
                            const hasConditionTag = matchesCondition(item.condition, allTags, true);
                            if (
        ...
        if (applyCompletionFilters && blockComplete && completionStatus === "complete")
            reasons.push({
                completionStatus: "Status: Complete",
                _filterType: "blockComplete",
            });
        if (applyCompletionFilters && blockOngoing && completionStatus === "ongoing")
            reasons.push({
        ...
        if (applyChapterFilters && (config.minChapters != null || config.maxChapters != null)) {
            const chapterInfo = getChapterInfo(ddElements);
            if (chapterInfo && chapterInfo.current != null) {
                const chapters = chapterInfo.current;
                let blocked = false;
                if (config.minChapters != null && chapters < config.minChapters) blocked = true;
                if (config.maxChapters != null && chapters > config.maxChapters) blocked = true;
                if (blocked)
                    reasons.push({
        ...
        if (
            ...
            if (monthsSinceUpdate != null && monthsSinceUpdate > config.maxMonthsSinceUpdate) {
        ...
        if (applyAuthorFilters && authors.length > 0 && authorBlacklist.has(authors[0])) {
            reasons.push({
                author: authors[0],
        ...
        allTags.forEach((tag) => {
            ...
            if (blockResult.blocked) {
                ...
                if (blockResult.type === "conditional" && blockResult.conditionDisplay) {
                    const op = blockResult.operator === "unless" ? "without" : blockResult.operator;
    ...
    function selectFromBlurb(blurbElement) {
        if (blurbElement.classList.contains("collection")) {
            ...
            const title =
                blurbElement.querySelector(".header .heading a:first-child")?.textContent.trim() ||
                "";
            const summary =
                blurbElement.querySelector("blockquote.summary")?.textContent.trim() || "";
            const authors = Array.from(blurbElement.querySelectorAll("a.owner")).map((a) =>
                a.textContent.toLowerCase().trim(),
            );
            return {
                authors: authors,
        ...
        return {
            authors: selectTextsIn(blurbElement, "a[rel=author]").map((a) => a.toLowerCase()),
            ...
            language: (ddElements.language ? ddElements.language.textContent.trim() : "")
                .toLowerCase()
                .trim(),
