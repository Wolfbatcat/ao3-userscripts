...
(function($) {
    ...
    function injectMenuRegistration() {
        const fn = function() {
            function waitForMenu(attempts = 20, interval = 250) {
                if (window.AO3UserScriptMenu && typeof window.AO3UserScriptMenu.register === 'function') {
                    ...
                    console.error('[AO3 CommentFormatting] Failed to find AO3UserScriptMenu API (injected)');
                }
            }
            waitForMenu();
        };
        ...
        script.remove();
        // Listen for the menu click event from the page context
        window.addEventListener('ao3commentformatting-open', function() {
    ...
    // adding the button bar & preview box for the New/Edit Work/Chapter form
    if ($('#work_summary, #work_notes, #work_endnotes, #chapter_summary, #chapter_notes, #chapter_endnotes').filter('textarea').length > 0) {
        $('#work_summary, #work_notes, #work_endnotes, #chapter_summary, #chapter_notes, #chapter_endnotes').filter('textarea')
