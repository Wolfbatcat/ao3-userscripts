...
(function () {
  ...
  const ThemeDetector = {
    ...
    getDialogStyles() {
      ...
      // Fall back to fieldset colors if dialog detection failed
    ...
    getBlurbStyles() {
      ...
      // Blurb is a base style - use hardcoded fallbacks only
      this.cache.blurb = {
        backgroundColor: styles?.backgroundColor && !this._isTransparent(styles.backgroundColor)
          ? styles.backgroundColor
          : "#f5f5f5",
        borderColor: styles?.borderColor && !this._isTransparent(styles.borderColor)
          ? styles.borderColor
          : "#d1d1d1",
        borderWidth: styles?.borderWidth || "1px",
        borderRadius: styles?.borderRadius || "8px",
    ...
    getFieldsetStyles() {
      ...
      // Fall back to blurb colors if fieldset detection failed
      let backgroundColor = styles?.backgroundColor;
      let borderColor = styles?.borderColor;
  ...
  window.AO3MenuHelpers = {
    ...
    injectSharedStyles() {
      ...
      if (
        this.themeDetector.isMobile &&
        !stylesInjected &&
        (this.themeDetector._isTransparent(dialogTheme.backgroundColor) ||
          this.themeDetector._isTransparent(buttonTheme.backgroundColor))
      ) {
        this.themeDetector.retryOnMobile();
        // Continue with fallback colors for now
      }
