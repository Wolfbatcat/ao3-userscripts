...
(function () {
     ...
     * Populates module-level FORMATTER_CONFIG.
     */
    ...
    /** Persist current FORMATTER_CONFIG to localStorage. */
    ...
    /**
     * Applies paragraph-level styles (width, font size, alignment, gap)
     * via an injected style element. Only activates on works/chapters pages.
     * Cascades to applySiteWideStyles afterward.
     */
    ...
    /**
     * Generates site-wide CSS rules for fonts, weights, code styles,
     * and date formatting. Inject into ao3-sitewide-style element.
     * Cascades to applyColorStyles afterward.
     */
    function applySiteWideStyles() {
        ...
        if (codeRules.length === 0) {
            ...
            if (expandCodeFontUsage) {
                rules.push(`textarea, #workskin textarea { font-family: monospace !important; }`);
            }
        }
    ...
    /**
     * Injects CSS custom properties for user-configured colors
     * (background, text, header, accent, logo) into ao3-color-style element.
     * Removes the style element when all color fields are blank.
     */
    function applyColorStyles() {
        ...
        if (!backgroundColor && !textColor && !headerColor && !accentColor && !logoColor) {
            if (colorStyle) {
                colorStyle.remove();
            }
            return;
        ...
        if (backgroundColor) rootVars.push(`--background-color: ${backgroundColor}`);
...
.userstuff h3 {
    ...
    const removeParaIndentation = (() => {
        return function () {
            ...
            document
                .querySelectorAll("#workskin .userstuff:not([data-formatter-indent-fixed])")
                .forEach((userstuff) => {
                    ...
                    userstuff.querySelectorAll("p").forEach((p) => {
                        ...
                        while (
                            ...
                            if (
                                ["br", "hr", "div", "blockquote", "ul", "ol", "table"].includes(tag)
                            )
                                break;
    ...
    /**
     * Builds and displays the Site Wizard settings dialog using MHL controls.
     * Reads current FORMATTER_CONFIG, renders sections for fonts, formatting,
     * colors, and saves changes on submit. Also handles import/export/reset.
     */
    function showFormatterMenu() {
        ...
        if (!window.AO3MenuHelpers) {
            ...
            alert("Error: Menu Helpers library not loaded. Please check your userscript manager.");
            return;
        }

        window.AO3MenuHelpers.removeAllDialogs();

        const dialog = window.AO3MenuHelpers.createDialog("🪄 Site Wizard Settings 🪄", {
            maxWidth: "700px",
        });

        // Site-Wide Display Section
        const siteSection = window.AO3MenuHelpers.createSection("📱 Site-Wide Display");
        ...
        const workSection = window.AO3MenuHelpers.createSection("📖 Work Formatting");
        ...
        const workFontSize = window.AO3MenuHelpers.createSliderWithValue({
            ...
            unit: "%",
            tooltip: "Size relative to general font size",
        });
        ...
        const lineSpacing = window.AO3MenuHelpers.createNumberInput({
            ...
            min: 0,
            step: 0.1,
            tooltip: "Vertical space between paragraphs (multiplier). Default is 1.286.",
        });

        const alignSpacingRow = window.AO3MenuHelpers.createTwoColumnLayout(textAlign, lineSpacing);
        ...
        const elementSection = window.AO3MenuHelpers.createSection("🎯 Element-Specific Fonts");
        ...
        const headerRow = window.AO3MenuHelpers.createTwoColumnLayout(headerFont, headerWeight);
        ...
        const codeRow = window.AO3MenuHelpers.createTwoColumnLayout(codeFontSize, codeFontStyle);
        elementSection.appendChild(codeRow);

        const expandCodeFont = window.AO3MenuHelpers.createCheckbox({
            id: "expand-code-font-checkbox",
            label: "Apply code font to textareas",
            checked: FORMATTER_CONFIG.expandCodeFontUsage,
            tooltip:
                "Applies code font to all textareas such as comments and text input fields. Requires a code/monospace font to be specified above.",
        });
        ...
        backgroundGroup.appendChild(
            window.AO3MenuHelpers.createLabel("Background Color", "sw_background_color"),
        );
        ...
        const accentLabel = window.AO3MenuHelpers.createLabel("Accent Color ", "sw_accent_color");
        ...
        const firstRow = window.AO3MenuHelpers.createTwoColumnLayout(backgroundGroup, textGroup);
        colorSection.appendChild(firstRow);

        const secondRow = window.AO3MenuHelpers.createTwoColumnLayout(headerGroup, accentGroup);
        colorSection.appendChild(secondRow);

        const logoGroup = window.AO3MenuHelpers.createSettingGroup();
        const logoLabel = window.AO3MenuHelpers.createLabel("Logo Color ", "sw_logo_color");
        ...
        dialog.appendChild(
            window.AO3MenuHelpers.createImportExportRow({
                ...
                exportData: () => FORMATTER_CONFIG,
                exportPrefix: "ao3_site_wizard_config",
                onImport: (file) => {
        ...
        // Event Handlers
        const saveBtn = dialog.querySelector("#formatter-save");
        if (saveBtn)
            saveBtn.addEventListener("click", () => {
                ...
                FORMATTER_CONFIG.fixParagraphSpacing =
                    window.AO3MenuHelpers.getValue("fix-paragraph-spacing-checkbox") ?? false;
                FORMATTER_CONFIG.removeIndentation =
                    window.AO3MenuHelpers.getValue("remove-indentation-checkbox") ?? false;
                ...
                FORMATTER_CONFIG.logoColor = window.AO3MenuHelpers.getValue("sw_logo_color") || "";
                ...
                if (
                    ...
                    alert("Must specify a Code/Monospace Font.");
                    return;
                }
        ...
        const cancelBtn = dialog.querySelector("#formatter-cancel");
        if (cancelBtn)
    ...
    /**
     * Initializes styles. If document.head exists, applies immediately.
     * Otherwise watches for head with MutationObserver (run-at document-start guard).
     */
    function initStyles() {
        if (document.head) {
            applyParagraphWidth();
    ...
    /** Run paragraph spacing fix if config enables it and on works page. */
    function runParagraphSpacingFixIfEnabled() {
        if (FORMATTER_CONFIG.fixParagraphSpacing && WORKS_PAGE_REGEX.test(window.location.href)) {
            fixParagraphSpacing();
        }
    }

    /** Run indentation removal if config enables it and on works page. */
    function runIndentRemovalIfEnabled() {
        if (FORMATTER_CONFIG.removeIndentation && WORKS_PAGE_REGEX.test(window.location.href)) {
