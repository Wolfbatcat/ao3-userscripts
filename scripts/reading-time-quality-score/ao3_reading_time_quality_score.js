...
(function () {
    ...
    const isMyContentPage = (username) => {
        ...
        const readingsPattern = new RegExp(`^/users/${escapedUsername}/readings(?:/|$)`);
    ...
    /** Extract work ID from a blurb element's title link. */
    const getWorkIdFromElement = (workElement) => {
        if (!workElement) return null;
        const titleLink = workElement.querySelector(".header .heading a:first-child");
        if (!titleLink) return null;
        const match = titleLink.href.match(/\/works\/(\d+)/);
        return match ? match[1] : null;
    };

    /** Extract work ID from URL or chapter page subscription form. */
    const getWorkIdFromUrl = () => {
        ...
        if (subInput && subInput.value) {
            const workTypeInput = document.querySelector(
    ...
    /** Parse numeric value from a DD element, handling comma separators. */
    ...
    const saveAllSettings = () => {
        if (typeof Storage !== "undefined") {
            ...
            localStorage.setItem("ao3_reading_quality_config", JSON.stringify(configToSave));
        }
    };
    ...
    /** Build a Set of work IDs whose scores should be hidden. */
    ...
    /** Save a single config key and persist. */
    function saveSetting(key, value) {
        CONFIG[key] = value;
        saveAllSettings();
    }

    /** Reset all config to DEFAULTS, including v4 migration values. */
    ...
    /** Migrate config to v4 if coming from an older version. */
    ...
    /** Check whether the page has countable stats elements and set flags. */
    ...
    /** Calculate word-length-adjusted kudos/hits ratio. */
    ...
    /** Calculate reading time and quality score for all work stats on page. */
    const calculateMetrics = (
        ...
        allStats.forEach((statsElement) => {
            ...
            const needsReadingTime = allowCalculation && CONFIG.enableReadingTime && !readtimeDt;
            const needsScore =
                allowCalculation &&
            ...
            if (needsReadingTime) {
                ...
                if (CONFIG.useIcons) {
                    ...
                    ddStyle += " border-radius: 4px; font-size: inherit; line-height: inherit;";
                    if (CONFIG.colorStyle === "background") {
            ...
            if (needsScore) {
                if (CONFIG.hideScoreWorksEnabled && CONFIG._hideScoreWorksSet instanceof Set) {
                    let workId = parentLi ? getWorkIdFromElement(parentLi) : null;
                    if (!workId) {
                        workId = getWorkIdFromUrl();
                    }
            ...
            if (CONFIG.hideWorksEnabled && parentLi) {
                ...
                if (shouldHide) {
                    if (parentLi.hasAttribute("kudospercent")) {
                        const displayScore = parseFloat(parentLi.getAttribute("kudospercent"));
                        parentLi.style.display = displayScore < CONFIG.hideWorksScore ? "none" : "";
                    } else {
    ...
    /** Calculate reading time only (menu-triggered). */
    const calculateReadtime = () => {
        if (!countable || !CONFIG.enableReadingTime) return;
        calculateMetrics(null, false, true);
    };

    /** Calculate quality score only (menu-triggered). */
    const countRatio = () => {
    ...
    /** Sort work elements by quality score in ascending or descending order. */
    ...
    /** Restore work elements to their original DOM order. */
    ...
    /** Update colors on existing reading time and score elements without recalculating. */
    ...
    /** Update existing chapter time display to match current config. */
    const updateExistingChapterTimeStyles = () => {
        ...
        existingStats.forEach((statsElement) => {
            ...
            if (statsElement.classList.contains("ao3-chapter-stats-default")) {
                wordCountText = statsElement.textContent.match(/(\d{1,3}(?:,\d{3})*|\d+) words/);
            } else if (
                statsElement.classList.contains("ao3-chapter-stats-colored") ||
                statsElement.classList.contains("ao3-chapter-stats")
            ) {
                wordCountText = statsElement.textContent.match(/(\d{1,3}(?:,\d{3})*|\d+) words/);
            } else if (statsElement.classList.contains("ao3-chapter-stats-timeonly")) {
                return;
            }
    ...
    /** Calculate and display word count + reading time at the start of each chapter. */
    ...
    function isAllowedMenuPage() {
        ...
        if (/^\/users\/[^\/]+\/bookmarks(\/|$)/.test(path) || /^\/bookmarks(\/|$)/.test(path))
            return true;
        if (/^\/users\/[^\/]+\/pseuds\/[^\/]+\/bookmarks(\/|$)/.test(path)) return true;
    ...
    /** Inject icon stylesheet for reading time and score labels. */
    const addIconStyles = () => {
    ...
    const showSettingsPopup = () => {
        ...
        const dialog = window.AO3MenuHelpers.createDialog("⏱️ Reading Time & Quality Score ⭐", {
            maxWidth: "600px",
        });
        ...
        readingTimeSubsettings.style.display = CONFIG.enableReadingTime ? "" : "none";
        ...
        qualityScoreSubsettings.style.display = CONFIG.enableQualityScore ? "" : "none";
        ...
        autoCalculateSubsettings.style.display = CONFIG.alwaysCountQualityScore ? "" : "none";
        ...
        const excludeMyContentSubsetting = document.createElement("div");
        ...
        const excludeCheckbox = window.AO3MenuHelpers.createCheckbox({
        ...
        excludeMyContentSubsetting.appendChild(excludeCheckbox);
        ...
        const hideWorksConditional = window.AO3MenuHelpers.createConditionalCheckbox({
            id: "hideWorksEnabled",
        ...
        const hideScoreWorksConditional = window.AO3MenuHelpers.createConditionalCheckbox({
        ...
        const visualSection = window.AO3MenuHelpers.createSection("🎨 Visual Styling");
        ...
        chapterTimeStyleGroup.style.display = CONFIG.enableChapterStats ? "" : "none";
        ...
        const colorsRow1 = window.AO3MenuHelpers.createTwoColumnLayout(
            window.AO3MenuHelpers.createTextInput({
                id: "colorGreen",
                label: "Green",
        ...
        const colorsRow2 = window.AO3MenuHelpers.createTwoColumnLayout(
            window.AO3MenuHelpers.createTextInput({
                id: "colorRed",
                label: "Red",
        ...
        twoColumnColors.appendChild(colorsRow1);
        twoColumnColors.appendChild(colorsRow2);
        ...
        const hideMetricsCheckbox = window.AO3MenuHelpers.createCheckbox({
            ...
            tooltip: "Hide metrics (hits, kudos, bookmarks, comments) from blurbs",
            inGroup: false,
        });
        ...
        const hideBookmarksCommentsRow = window.AO3MenuHelpers.createTwoColumnLayout(
            window.AO3MenuHelpers.createCheckbox({
                id: "hideBookmarks",
                label: "Bookmarks",
        ...
        fragment.appendChild(
            window.AO3MenuHelpers.createImportExportRow({
                ...
                exportData: () => {
                    const { _hideScoreWorksSet, ...exportable } = CONFIG;
                    return exportable;
                },
                exportPrefix: "ao3_reading_quality_config",
                onImport: (file) => {
        ...
        dialog.querySelector("#enableReadingTime").addEventListener("change", (e) => {
            readingTimeSubsettings.style.display = e.target.checked ? "" : "none";
        });

        dialog.querySelector("#enableChapterStats").addEventListener("change", (e) => {
            chapterTimeStyleGroup.style.display = e.target.checked ? "" : "none";
        });

        dialog.querySelector("#enableQualityScore").addEventListener("change", (e) => {
            qualityScoreSubsettings.style.display = e.target.checked ? "" : "none";
        });

        dialog.querySelector("#alwaysSortQualityScore").addEventListener("change", (e) => {
            excludeMyContentSubsetting.style.display = e.target.checked ? "" : "none";
        });
        ...
        colorStyleSelect.addEventListener("change", () => {
            ...
            colorPickerSettings.style.display = selectedStyle !== "none" ? "" : "none";
            textColorContainer.style.display = selectedStyle === "background" ? "" : "none";
        });
        ...
        dialog.querySelector("#useCustomIconColor").addEventListener("change", (e) => {
            customIconColorPicker.style.display = e.target.checked ? "" : "none";
        });

        dialog.querySelector("#useNormalization").addEventListener("change", (e) => {
            ...
            const userMaxScoreContainer = dialog.querySelector("#userMaxScoreContainer");
            ...
            const hideWorksScoreLabel = dialog.querySelector("#hideWorksScoreLabel");
            if (isNormalizationEnabled) {
        ...
        dialog.querySelector("#alwaysCountQualityScore").addEventListener("change", (e) => {
        ...
        dialog.querySelector("#saveButton").addEventListener("click", () => {
            let userMaxScoreValue = parseFloat(dialog.querySelector("#userMaxScore").value);
            let thresholdLowValue = parseFloat(dialog.querySelector("#colorThresholdLow").value);
            let thresholdHighValue = parseFloat(dialog.querySelector("#colorThresholdHigh").value);
            const isNormalizationEnabled = dialog.querySelector("#useNormalization").checked;

            CONFIG.enableReadingTime = dialog.querySelector("#enableReadingTime").checked;
            CONFIG.enableQualityScore = dialog.querySelector("#enableQualityScore").checked;
            CONFIG.enableChapterStats = dialog.querySelector("#enableChapterStats").checked;
            CONFIG.alwaysCountReadingTime = dialog.querySelector("#alwaysCountReadingTime").checked;
            CONFIG.wpm = parseInt(dialog.querySelector("#wpm").value);
            CONFIG.readingTimeLvl1 = parseInt(dialog.querySelector("#readingTimeLvl1").value);
            CONFIG.readingTimeLvl2 = parseInt(dialog.querySelector("#readingTimeLvl2").value);
            ...
            CONFIG.alwaysSortQualityScore = dialog.querySelector("#alwaysSortQualityScore").checked;
            ...
            const normalizationChanged = CONFIG.useNormalization !== isNormalizationEnabled;
            ...
            const hideScoreWorksChanged =
                ...
                CONFIG.hideScoreWorks !== dialog.querySelector("#hideScoreWorks-input").value;

            CONFIG.hideScoreWorksEnabled = dialog.querySelector("#hideScoreWorksEnabled").checked;
            CONFIG.hideScoreWorks = dialog.querySelector("#hideScoreWorks-input").value;
            buildHideScoreWorksSet();
            ...
            const existingIconStyles = document.getElementById("ao3-userscript-icon-styles");
            ...
            const readingTimeDisabled = CONFIG.enableReadingTime && !CONFIG.alwaysCountReadingTime;
    ...
    /** Register menu items in the shared Userscripts dropdown. */
    ...
    /** Initialize: detect page type, run calculations, set up menu. */
    const init = () => {
        checkCountable();
